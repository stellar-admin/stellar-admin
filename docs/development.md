...
EF Core and its SQLite integration tests are back in `StellarAdmin.slnx` and build/release verification. `AddEfCoreResource<TContext, TEntity>` currently supplies a read-only index with key discovery, paging, and sorting through the shared controller. EF expression configuration and CRUD are subsequent checkpoints. Identity and `sandbox/IdentitySimplePlayground` remain detached and still reference removed APIs.
