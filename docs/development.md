...
`AddResource<TResource>()` automatically adds a sidebar link to the resource index through one shared Dashboard provider. The link label follows `PluralLabel` by default. `resource.SidebarItem(item => ...)` can override its `Label`, `Group`, `Order`, and `Visible` values. Resources default to the Resources group, sort by `Order` within a group, and keep registration order for ties. `AddEfCoreResource<TContext, TEntity>()` uses the same registration path. Hidden links leave resource routes available.
