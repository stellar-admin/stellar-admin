...
- **Builders are facades over a single options object** registered through the options pattern (`services.Configure<...>`), so state is testable and plays with `IOptions<>`.
...
- **Per-column chaining shape** — `columns.Add(u => u.Email).Title("E-mail")` (EF style, current lean) vs a second config-lambda argument.
