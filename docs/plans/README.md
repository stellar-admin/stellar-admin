...
| [resource-configuration-and-controller-unification](resource-configuration-and-controller-unification.md) | active | Resource reset committed on `resource-redesign`; EF, Identity, and their sample remain detached. Rebuild the standalone core in eight steps, proving action-specific view models immediately after basic CRUD, before index features and integration adaptation. Step 1 registration and naming is implemented and tested. The working index page is next. |
