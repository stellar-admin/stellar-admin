...
| [resource-configuration-and-controller-unification](resource-configuration-and-controller-unification.md) | needs revision | Phase 1 implementation reverted at the user’s request. EF, Identity, and their sample are detached on `resource-redesign`. The old resource orchestration and its test suites have been removed. Rendering and layout definitions remain. Reconsider around a standalone resource foundation before adapting the integrations. Action-specific form models remain deferred. |
