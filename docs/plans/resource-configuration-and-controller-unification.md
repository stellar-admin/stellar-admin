...
## Separate form editor options and runtime editors — 2026-09-23

Changed the field API to `UseEditor<TOptions>(options => ...)`. An options class derives from `EditorOptions` and implements `IFieldEditorOptions<TEditor>`; its associated editor implements `IFieldEditor<TOptions>`. Resource options retain only the configured options and editor type. The shared controller creates each editor for a form request with `ActivatorUtilities`, passes that field's options to its constructor, resolves other constructor dependencies from request DI, and carries the editor's template name and prepared data into the form view. Fields without a selected editor still use MVC metadata templates. The custom Dashboard HTTP fixture now proves that an application editor receives both its options and an injected host service, while its Razor template reads the options and prepared data separately.

`ReferenceLookupEditorOptions` now owns the selected lookup provider. `ReferenceLookupEditor` receives those options and a narrow DI lookup-provider resolver; it no longer inherits `EditorOptions` or takes `IServiceProvider`. The old `ResourceEditor` type was removed. Updated DashboardPlayground and consumer setup guidance to use `UseEditor<ReferenceLookupEditorOptions>`. The lookup provider remains application-registered and is selected per field.

Verification: the serial Debug solution build passed with 12 existing warnings and no errors. Dashboard HTTP (142) and EF SQLite HTTP (56) integration tests passed. CSharpier formatted the touched C# files, and the consumer skill generator `--check` reported no drift. No browser check, commit, or push was requested.

## Restore edit-page deletion — 2026-09-24

Restored the edit-page Delete trigger and the existing server-rendered form dialog when deletion is enabled. The edit dialog owns a separate antiforgery-protected POST form outside the edit form. The index retains its shared confirmation dialog and HTMX row form. Both use the same resource-level delete title, message, confirmation, and cancellation labels. The edit form marks its delete POST with a fixed origin value so a rejected delete redirects back to Edit with the operation's error messages in the validation summary. This avoids rendering Index at the Delete POST URL and leaves the existing index rejection behavior unchanged. Successful deletion still redirects to Index.

Added four Dashboard HTTP scenarios covering the edit dialog's markup and antiforgery token, omission when deletion is disabled, successful deletion, and rejected deletion with encoded errors after the redirect. Updated the Dashboard consumer setup notes. The edit-page Delete trigger now uses the destructive button variant. Removed the unused `sa-resource-form-delete` rules and rebuilt Dashboard CSS successfully. Verification: the Dashboard HTTP executable passed all 146 tests, and the EF SQLite HTTP executable passed all 56 tests before the button styling change. CSharpier formatted the three touched C# files and `git diff --check` passed. No browser check or full solution run was performed.

## Edit-owned Delete trigger — 2026-09-24

Moved the Delete trigger from the shared `_FormPage` into the resource Edit view through a new `form-leading-actions` slot. The slot preserves the left-side footer placement, while the existing `pre-form-actions` and `post-form-actions` slots stay around the right-side Cancel and submit controls. The separate `_FormDeleteDialog` remains outside the resource edit form. The Create view still uses the shared form layout without a Delete control. Extended the Dashboard HTTP checks for the trigger's footer placement and Create-page omission. The detached Identity project and its historical design notes remain untouched pending its separate adaptation.

Verification: the Dashboard HTTP suite passed all 147 tests and the EF SQLite HTTP suite passed all 56 tests before narrowing the edit-view changes to Dashboard. An Identity build attempt confirmed that project is still detached: its controllers reference removed resource APIs, and its options inherit from now-sealed resource options. The Dashboard suite was rerun after the narrower edit. `git diff --check` passed. No browser check was performed.
