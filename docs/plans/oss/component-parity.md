...
- [ ] `TagHelper` under `src/StellarAdmin.TagHelpers/TagHelpers/<Component>/` — inherit `StellarAdminTagHelperBase`
...
- [ ] Component CSS — structural `.sa-<component>*` rules in `src/StellarAdmin.TagHelpers/Client/css/components.css`
      ...
      **No styling literals in C#** beyond the sanctioned set in the `@source inline()` safelist
      at the top of `components.css` (see the workspace OSS development guide).
...
- [ ] `npm run build` (js + css) from `src/StellarAdmin.TagHelpers/Client/`; format with CSharpier + oxfmt.
...
- [ ] Report changes and verification per affected repo. Commit only when authorized in the conversation.
...
- **Theme rules** — every `cn-<name>` in shadcn maps to a `sa-<name>` class whose theme-specific
  ...
  same class name (structural wins conflicts — see the workspace OSS development guide's precedence contract). A name with
...
- **Form** — react-hook-form/zod; StellarAdmin.TagHelpers solves this via `asp-for` + Field/validation.
