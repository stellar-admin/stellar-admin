...
public partial class Program
{
    ...
    public static async Task Main(string[] args)
    {
        ...
        foreach (var themeFile in themeFiles)
        {
            ...
            var outputFile = Path.Combine(themesFolder, $"shadcn.{lowerThemeName}.css");
            if (
                File.Exists(outputFile)
                && !(await File.ReadAllTextAsync(outputFile)).StartsWith(
                    "/* Generated by util/ThemeGenerator from shadcn "
                )
            )
            {
                throw new InvalidOperationException(
                    $"Refusing to overwrite hand-authored CSS: {outputFile}"
                );
            }
