...
Ledger is hand-authored in `Client/css/themes/ledger.css`. ThemeGenerator owns only the upstream-derived theme files. Its hardcoded source list must never include Ledger. Consumer applications select Ledger by linking its bundle; there is no separate tag helper API.
