// Real-component regressions for Meridian's selection, focus, and composite geometry.
// node util/visual-regression/verify-meridian.mjs http://localhost:5206
import assert from "node:assert/strict";
import { mkdir, readFile, writeFile } from "node:fs/promises";
import { launchBrowser, sleep } from "./browser.mjs";

const base = process.argv[2] ?? "http://localhost:5206";
const fontCss = process.env.MERIDIAN_FONT_CSS
  ? await readFile(process.env.MERIDIAN_FONT_CSS, "utf8")
  : null;
const output = new URL("./snapshots/meridian/", import.meta.url);
...
async function sample(name, mode) {
  ...
  await send("Page.navigate", {
    url: `${base}/DocsStatic?name=${name}&layout=_CleanLayout&theme=meridian&mode=${mode}`,
  });
  assert.ok(await loaded, `${name}: loaded`);
  assert.ok(
    await evaluate(`!!document.querySelector('link[href*="stellar-admin.meridian"]')`),
    "Meridian selected",
  );
...
async function prepareFonts(name) {
  if (fontCss) {
    await evaluate(`(() => {
      ...
      return Promise.all([
        document.fonts.load('600 14px "Instrument Sans"'),
        document.fonts.load('400 13px "Work Sans"'),
        document.fonts.load('500 13px "Work Sans"'),
        document.fonts.load('600 13px "Work Sans"'),
    ...
    for (const width of [1280, 390]) {
      ...
      assert.equal(
        await evaluate(`(() => {
          ...
          probe.style.borderColor = 'var(--sa-meridian-line-strong)';
      ...
      assert.equal(
        await evaluate(`getComputedStyle(document.querySelector('.sa-card-title')).fontFamily.includes('Instrument Sans')`),
        true,
        "card headings use the display family",
      );
      assert.equal(
        await evaluate(`getComputedStyle(document.querySelector('.sa-card-description')).fontFamily.includes('Work Sans')`),
        true,
      ...
      assert.equal(
        ...
        mode === "dark",
        "dark editable fields rise above cards; light fields share the card surface",
      );
      ...
      console.log(
        `PASS Meridian ${mode}/${width}: focus, selection, grouped validation, native inputs and overlay containment`,
      );
    }
  }
