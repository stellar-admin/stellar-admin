// Real-component regressions for Aurora's selection, focus, and composite geometry.
// node util/visual-regression/verify-aurora.mjs http://localhost:5206
import assert from "node:assert/strict";
import { mkdir, readFile, writeFile } from "node:fs/promises";
import { launchBrowser, sleep } from "./browser.mjs";

const base = process.argv[2] ?? "http://localhost:5206";
const fontCss = process.env.AURORA_FONT_CSS
  ? await readFile(process.env.AURORA_FONT_CSS, "utf8")
  : null;
const output = new URL("./snapshots/aurora/", import.meta.url);
...
async function sample(name, mode) {
  ...
  await send("Page.navigate", {
    url: `${base}/DocsStatic?name=${name}&layout=_CleanLayout&theme=aurora&mode=${mode}`,
  });
  assert.ok(await loaded, `${name}: loaded`);
  assert.ok(
    await evaluate(`!!document.querySelector('link[href*="stellar-admin.aurora"]')`),
    "Aurora selected",
  );
  await prepareFonts(name);
  assert.ok(
    await evaluate(
      "getComputedStyle(document.documentElement).getPropertyValue('--sa-aurora-accent').trim()",
    ),
    "Aurora stylesheet loaded with its foundation tokens",
  );
}
async function prepareFonts(name) {
  if (fontCss) {
    await evaluate(`(() => {
      ...
      return Promise.all([
        document.fonts.load('400 13px "Archivo"'),
        document.fonts.load('500 13px "Archivo"'),
...
try {
  ...
  for (const mode of ["light", "dark"]) {
    for (const width of [1280, 390]) {
      ...
      assert.equal(
        await evaluate(
          `getComputedStyle(document.querySelector('.sa-card-title')).fontFamily.includes('Archivo')`,
        ),
        true,
      ...
      assert.equal(
        await evaluate(
          `getComputedStyle(document.querySelector('.sa-card-description')).fontFamily.includes('Archivo')`,
        ),
        true,
      ...
      assert.equal(
        await evaluate(
          `Array.from(document.querySelectorAll('.sa-slider-track, .sa-slider-range, .sa-slider-thumb')).every(el => getComputedStyle(el).borderRadius === '0px')`,
        ),
        true,
        "slider track, range and thumb are square",
      );
      assert.notEqual(
      ...
      assert.match(
        ...
        /2px/,
        "flat selected segment preserves its 2px keyboard ring",
      );
      ...
      console.log(
        `PASS Aurora ${mode}/${width}: focus, selection, grouped validation, native inputs and overlay containment`,
      );
    }
  }
