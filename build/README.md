...
Every run builds and validates Core, TagHelpers, and Dashboard, discovers and runs all solution TUnit unit and integration test projects, runs consumer reference checks, tests a temporary consumer app, and uploads all six package/symbol files. Source Link validation remains enabled. `build/smoke-test.sh` contains the consumer test.
