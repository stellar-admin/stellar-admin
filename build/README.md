...
Every run builds and validates Core, TagHelpers, Dashboard, Dashboard.Identity, and Dashboard.EntityFrameworkCore, discovers and runs all solution TUnit unit and integration test projects, runs consumer reference checks, tests a temporary consumer app, and uploads all ten package/symbol files. Source Link validation remains enabled. `build/smoke-test.sh` contains the consumer test.

## Test discovery

Both [Build and Test](../.github/workflows/ci.yml) and [Release](../.github/workflows/release.yml) use `dotnet test --solution StellarAdmin.slnx --no-build --configuration Release --minimum-expected-tests 1` after building. `global.json` selects Microsoft.Testing.Platform; new TUnit projects added to the solution participate automatically. No individual test-project list or legacy runner step remains. The five current test assemblies cover Core, TagHelpers, Dashboard builders, Dashboard rendering/binding, and EF resource integration. Hosted integration tests use isolated temporary SQLite databases; see [integration test isolation](../docs/development.md#integration-test-isolation).
